...
class VmMigrationClient(metaclass=VmMigrationClientMeta):
    ...
    @staticmethod
    def replication_cycle_path(project: str,location: str,source: str,migrating_vm: str,replication_cycle: str,) -> str:
        """Returns a fully-qualified replication_cycle string."""
        return "projects/{project}/locations/{location}/sources/{source}/migratingVms/{migrating_vm}/replicationCycles/{replication_cycle}".format(project=project, location=location, source=source, migrating_vm=migrating_vm, replication_cycle=replication_cycle, )

    @staticmethod
    def parse_replication_cycle_path(path: str) -> Dict[str,str]:
        """Parses a replication_cycle path into its component segments."""
        m = re.match(r"^projects/(?P<project>.+?)/locations/(?P<location>.+?)/sources/(?P<source>.+?)/migratingVms/(?P<migrating_vm>.+?)/replicationCycles/(?P<replication_cycle>.+?)$", path)
        return m.groupdict() if m else {}
    ...
    def list_replication_cycles(self,
    ...
    def get_replication_cycle(self,
